...
from keras.layers import Input, Conv2D, Lambda,  Flatten, Dropout,Reshape,UpSampling2D, Dense
...
def digit_encoder(param, name = 'digit_encoder'):
    input_shape = (param.resolution, param.resolution, param.image_channels)
    model = Sequential(name =name)
    model.add(Conv2D(96, kernel_size=11, strides=(2, 2), padding='same', activation='relu', input_shape=input_shape))
...
def encoder2(param,name = 'encoder2'):
...
def encoder3(param,name = 'encoder3'):
    #remove batchnorm, dropout and regularization
    input_shape = (param.resolution, param.resolution, param.image_channels)
    model = Sequential(name =name)
    model.add(Lambda(lambda img: img[:, :, :, ::-1] * 255.0, input_shape=input_shape))
    model.add(Lambda(subtract_mean))
    model.add(Lambda(conv2d_relu_,
                            arguments={'net_layers': param.caffenet_models_weights, 'layer': 0, 'layer_name': 'conv1', 'stride': param.conv1_stride,
                                       'pad': 'SAME'}))
    for i in range(param.num_conv_layers):
        model.add(Conv2D(param.conv_ch, (3, 3), padding='same', kernel_initializer="glorot_normal", activation='relu', strides=(2, 2)))
    model.add(Flatten())
    model.add(Dense(units=param.num_voxels))
    return model

def encoder_nopretrain(param, name='encoder'):
    input_shape = (param.resolution, param.resolution, 3)
    model = Sequential(name =name)
    model.add(Lambda(lambda img: img[:, :, :, ::-1] * 255.0, input_shape=input_shape))
    model.add(Lambda(subtract_mean))
    model.add(Conv2D(256, kernel_size=9, padding='valid', kernel_initializer='glorot_normal', activation='relu')) #idk why i have 256 instead of 96. Pretrained layer has 96 filters
    model.add(BatchNormalization(axis=-1))
    for i in range(param.num_conv_layers):
        model.add(Conv2D(param.conv_ch, (3, 3), padding='same', kernel_initializer="glorot_normal", activation='relu',
                                kernel_regularizer=l1_l2(param.conv_l1_reg,param.conv_l2_reg), strides=(2, 2)))
...
def decoder_subset(param, voxel_subset, name = 'decoder'):
    input_shape = (param.num_voxels,)
    model = Sequential(name = name)
    model.add(Lambda(lambda input_voxels: tf.gather(input_voxels, voxel_subset, axis=1), input_shape=input_shape))
    model.add(dense_f2c_gl( out=[param.conv1_dim,param.conv1_dim,param.conv_ch],l1=param.fc_reg_l1,gl=param.fc_reg_gl,gl_n=param.fc_reg_gl_n,input_shape = input_shape,name='fc_d') )
    model.add(Reshape((param.conv1_dim, param.conv1_dim, param.conv_ch)))
    for i in range(param.num_conv_layers):
        model.add(Conv2D( param.conv_ch, (3, 3), padding='same',kernel_initializer="glorot_normal", activation='relu',kernel_regularizer=l1(param.conv_l1_reg )))
...
def encdec_vox_subset(NUM_VOXELS,RESOLUTION,encoder_model,decoder_model, voxel_subset):
    '''Returns encoder_decoder model where decoder uses a subset of the voxels

    WARINGING: Does not work, use decoder_subset instead

    Args:
        NUM_VOXELS: The number of voxels in the subset
        RESOLUTION: resolution of the square images
        encoder_model: A trained encoder model that predicts voxels
        decoder_model: An untrained decoder model
        voxel_subset: A list of indices of voxels to select
    '''
    ...
    # Get predicted voxels from encoder
    pred_voxel = encoder_model(input_img)
    pred_voxel_subset = Lambda(lambda t: tf.gather(t, voxel_subset, axis=1))(pred_voxel)
    input_voxel_subset = Lambda(lambda t: tf.gather(t, voxel_subset, axis=1))(input_voxel)
    rec_img_dec = decoder_model(input_voxel_subset)

    rec_img_encdec = decoder_model(pred_voxel_subset)
    pred_voxel_decenc = encoder_model(rec_img_dec)
